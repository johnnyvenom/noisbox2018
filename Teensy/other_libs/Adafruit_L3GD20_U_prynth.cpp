...
{
  Wire1.beginTransmission(L3GD20_ADDRESS);
  #if ARDUINO >= 100
...
byte Adafruit_L3GD20_Unified::read8(byte reg)
{
  ...
  Wire1.beginTransmission((byte)L3GD20_ADDRESS);
  #if ARDUINO >= 100
    Wire1.write((uint8_t)reg);
  #else
    Wire1.send(reg);
  #endif
  Wire1.endTransmission();
  Wire1.requestFrom((byte)L3GD20_ADDRESS, (byte)1);
  while (!Wire1.available()); // Wait for data to arrive.
  #if ARDUINO >= 100
    value = Wire1.read();
  #else
    value = Wire1.receive();
...
bool Adafruit_L3GD20_Unified::getEvent(sensors_event_t* event)
{
  ...
  while(!readingValid)
  {
    ...
    Wire1.beginTransmission((byte)L3GD20_ADDRESS);
    #if ARDUINO >= 100
      Wire1.write(GYRO_REGISTER_OUT_X_L | 0x80);
    #else
      Wire1.send(GYRO_REGISTER_OUT_X_L | 0x80);
    #endif
    if (Wire1.endTransmission() != 0) {
        // Error. Retry.
        continue;
    }
    Wire1.requestFrom((byte)L3GD20_ADDRESS, (byte)6);
